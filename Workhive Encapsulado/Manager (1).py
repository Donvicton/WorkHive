...
class Manager():
    ...
    def _get_users(self):
        return self.users  # Encapsulamento
    ...
    def menu_principal(self):
        ...
        print('999 para mostrar usuários cadastrados.')
        return int(input('\nSelect option: '))
    ...
    def show_users(self):
        for user in self._get_users().values():
            print(f"Username: {user.get_username()} || CPF: {user.get_cpf()} || Password: {user.get_password()}")
    ...
    def edit_user(self):
        ...
        if option == 1:
            ...
            self._get_users()[self._get_logged_username()].set_username(new_username)
        if option == 2:
            new_password = input('Digite uma nova senha: ')
            self._get_users()[self._get_logged_username()].set_password(new_password)
